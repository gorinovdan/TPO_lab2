...
public class LogTest {
    ...
    public static void setupMocks() {
        setup(ln, "src/test/resources/logCsv/Ln.csv");
        setup(log2, "src/test/resources/logCsv/Log2.csv");
        setup(log3, "src/test/resources/logCsv/Log3.csv");
        setup(log5, "src/test/resources/logCsv/Log5.csv");
    }
    ...
    void LogMocks(Double x, Double expected) {
        try {
            LogFunctionCalculator lfc = new LogFunctionCalculator(ln, log2, log3, log5);
    ...
    void LogRealLn(Double x, Double expected) {
        try {
            LogFunctionCalculator lfc = new LogFunctionCalculator(lnR, log2, log3, log5);
            double result = lfc.checkAndCalculate(x, eps);
            assertEquals(expected, result, Math.abs(result*percentage));
        } catch (ArithmeticException e) {
            assertEquals("value x should be > 0", e.getMessage());
    ...
    void LogRealLnLog2(Double x, Double expected) {
        try {
            LogFunctionCalculator lfc = new LogFunctionCalculator(lnR, log2R, log3, log5);
            double result = lfc.checkAndCalculate(x, eps);
            assertEquals(expected, result, Math.abs(result*percentage));
        } catch (ArithmeticException e) {
            assertEquals("value x should be > 0", e.getMessage());
    ...
    void LogRealLnLog2Log3(Double x, Double expected) {
        try {
            LogFunctionCalculator lfc = new LogFunctionCalculator(lnR, log2R, log3R, log5);
            double result = lfc.checkAndCalculate(x, eps);
            assertEquals(expected, result, Math.abs(result*percentage));
        } catch (ArithmeticException e) {
            assertEquals("value x should be > 0", e.getMessage());
    ...
    void LogAllReal(Double x, Double expected) {
        try {
            LogFunctionCalculator lfc = new LogFunctionCalculator(lnR, log2R, log3R, log5R);
            double result = lfc.checkAndCalculate(x, eps);
            assertEquals(expected, result, Math.abs(result*percentage));
        } catch (ArithmeticException e) {
            assertEquals("value x should be > 0", e.getMessage());
